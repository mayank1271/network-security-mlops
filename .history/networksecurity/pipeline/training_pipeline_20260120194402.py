...
class TrainingPipeline:
    ...
    def start_data_validation(self,data_ingestion_artifact:DataIngestionArtifact):
          try:
                ...
                data_validation=DataValidation(data_ingestion_artifact=data_ingestion_artifact,
                                               data_validation_congfig=data_validation_config)
    ...
    # my local artifacts is going to s3 bucket
    def sync_artifact_to_s3(self):
            try:
                aws_bucket_url=f"s3://{TRAINING_BUCKET_NAME}/artifact/{self.training_pipeline_config.timestamp}"
                self.s3_sync.sync_folder_to_s3(folder=self.training_pipeline_config.model_trainer_dir,aws_bucket_url=aws_bucket_url)
            except Exception as e:
                raise NetworkSecurityException(e,sys)
